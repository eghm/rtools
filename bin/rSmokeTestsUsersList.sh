# Run before rSmokeTestsTargetList.sh the file produced SmokeTestsUsers.txt is used by rSmokeTestTargetList.sh
...
#	echo `printf "$line,loadtester%03d" $i` >> SmokeTestsUsers.txt;
	echo `printf "$line:admin"` >> SmokeTestsUsers.txt;
    let "i+=1";
done;
rm SmokeTests.txt
