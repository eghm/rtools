# user, password, db
# mysqlTables.txt can be created with mysqlShotTables user password db
for f in $(cat $R_HOME/rtools/etc/mysqlTables.txt) ; do 
    ...
    perl $R_HOME/rtools/bin/mysqlDump2ImpexDateFormat.pl.pl -f $f.$3.impex.xml > $f.$3.tmp.xml
